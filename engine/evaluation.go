...
// Flip black's perspective to white
var FLIP_SQ = [2][64]int{
	{
...
func evaluate(pos Position) int {
    ...
    phase_score := get_phase_score(pos)	

    // get current game phase
    phase := get_phase(phase_score)
    ...
    if phase == middlegame {
        eval.score = (op_score * phase_score + eg_score * 
                (opening_phase_score - phase_score)) / opening_phase_score
...
// get game phase score
func get_phase_score(pos Position) int {
...
func get_phase(phase_score int) uint8 {
    // return opening phase if score > opening phase
    if phase_score > opening_phase_score {
        return opening
